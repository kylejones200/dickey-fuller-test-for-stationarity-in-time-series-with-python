...
def calculate_rolling_stats(df: pd.DataFrame, window: int=12) -> pd.DataFrame:
    ...
    return pd.DataFrame({'value': df['value'], 'rolling_mean': df['value'].rolling(window=window).mean(), 'rolling_std': df['value'].rolling(window=window).std()})

def test_stationarity(series: pd.Series) -> dict[str, Any]:
    """Perform Augmented Dickey-Fuller test."""
    result = adfuller(series)
    return {'adf_statistic': result[0], 'p_value': result[1], 'critical_values': result[4], 'is_stationary': result[1] <= 0.05}
